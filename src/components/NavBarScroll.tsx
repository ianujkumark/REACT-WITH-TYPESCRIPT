...
const NavBarScroll: React.FC = () => {
    ...
    const handleSign= () => {
        const newStatus = !LogInStatus; // compute the new value
    setLogInStatus(newStatus);
    setSignIn(newStatus ? "Welcome" : "Sign In");
    if(!newStatus)
        setsub("Subscribe")
        setsubstatus(false) //check status for everyhting related to the the chnage
    }
    ...
    const handlesub = () => {
        if(!LogInStatus)
        {
            ...
            return; //return is imp here
        }
        const temp = !substatus
        console.log(temp)
        setsubstatus(temp)
        setsub(temp ? "SUBSCRIBED!" : "Subscribe")
    }
  ...
  return (
    ...
    <div style={{ marginTop: "40px" }}>
      <nav
        className="nav top-navbar fixed-top fixed-top bg-white"
        style={{ padding: "10px" }}
      >
        <div className="container-fluid d-flex justify-content-between align-items-center">
          ...
          <div className="d-flex align-items-center">
            <ul className="nav align-items-center mb-0" style={{ gap: "30px" }}>
              ...
              <li className="nav-item">
                <button className="btn btn-dark me-2" onClick={handleSign}>{SignIn}</button>
                <button className="btn btn-danger" onClick={handlesub}>{sub}</button>
              </li>
            </ul>
          </div>
        </div>
